...
def api():
    ...
    if data['method'] == 'booking':
        ...
        cur.execute("SELECT * FROM offices WHERE number = %s;",
                    (office_number,))
        office = cur.fetchone()
        if office is None:
            db_close(conn, cur)
        ...
        cur.execute(
            "UPDATE offices SET tenant = %s WHERE number = %s;", (login, office_number))
        db_close(conn, cur)
        return {
            'jsonrpc': '2.0',
            'result': 'success',
    ...
    if data['method'] == 'cancellation':
        ...
        cur.execute(
            "UPDATE offices SET tenant = NULL WHERE number = %s;", (office_number,))
        db_close(conn, cur)
        return {
            'jsonrpc': '2.0',
            'result': 'success',
